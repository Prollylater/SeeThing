...
Mat<T> EgalizeHist(const Mat<T> &img, int power)
{
    ...
    Mat<float> cdf = hist.convertDt<float>();
    cdf /= (img.getRows() * img.getCols());
    // std::cout << cdf.rows << "colonnes" << cdf.cols;
    Mat<T> newimg(img);

    for (int j = 0; j < img.getRows(); i++)
    {
        for (int i = 0; i < img.getCols(); j++)
        {
            for (int c = 0; c < img.getChannels(); j++)
            {
                int cdfpix = static_cast<int>(img.at<T>(j, i, c));
...
Mat<T> applyFilterConv3(const Mat<T> &src, const Mat<T> &filter)
{
    Mat<T> inputImage = src.convertDt<float>();
    Mat<T> newimg(inputImage);

    int filterSize = filter.rows;

    for (int i = 1; i < inputImage.getCols() - 1; ++i)
    {
        for (int j = 1; j < inputImage.getRows() - 1; ++j)
        {
            for (int c = 0; c < inputImage.getChannels(); ++c)
            {
                ...
                for (int k = 0; k < filterSize; ++k)
                {
                    ...
                    {
                        //  if (i + k >= 0 && i + k< inputImage.rows && j + l >= 0 && j + l < inputImage.cols) {
                        sum += filter.atChannel(k, l, c) * inputImage.atChannel(j + k - 1, i + l - 1);
                        //  }
                    }
                }
                ...
                newimg.atChannel(j, i, c) = static_cast<float>(sum);
            }
        }
    }

    return newimg.convertDt<T>();
}
